  ...
  reducers: {
    ...
    setEnteredValue: (state, action) => {
      if (state.steps.length) {
        ...
        const isSuccess = action.payload === step.currentValue
...
export default playgroundSlice.reducer // из за экспорта по дефолту в файле где импорт можно назвать по другому этот редюсер playgroundReducer
